...
func _process(_delta: float) -> void:
	if AI_Pieces_setted and AI_CanRun and physics_controller.Sim_Controller_list[0].current_pitch_state.all_physic_object_list.size() > 0:
		...
		for i in PhysicsObjects_HomeTeam_IndexList.size():
			...
			Set_All_Plays_To_Simulate_AroundBall_Mode(num_steps, step_angle, max_force_steps, current_TeamSide, current_index)
		
		# Get list size
		var num_p = list_of_plays_to_simulate.size()
		
		if num_p <= 0:
			print("No piece close to ball, Getting Random Piece")
			# get the best piece to play
			current_index = Get_Index_Of_Closest_Piece_To_Ball(current_TeamSide)
		...
		for i in physics_controller.num_threads:
			...
			if final_index >= list_of_plays_to_simulate.size():
				...
				#print("final_index Corrected = ", final_index)
			
			for play in list_of_plays_to_simulate:
				if play.force_lerp == 0:
					print("play with force 0 index = ", play.player_index)
	...
	if list_of_plays_simulated.size() >= list_of_plays_to_simulate.size() - 1 and !list_play_sorted and list_separated:
		...
		var max_index = floor(list_of_plays_simulated.size() * 0.05)
		...
		var play_index = 0 #randi_range(0, max_index)
		print("play_index selected = ", play_index)
		print("play piece index = ", list_of_plays_simulated_Ordered[play_index].player_index)
		print("play direction = ", list_of_plays_simulated_Ordered[play_index].direction)
		print("play force_lerp = ", list_of_plays_simulated_Ordered[play_index].force_lerp)
...
func sort_by_score(size_ordered: int):
	...
	for i in size_ordered:
		...
		#
		#print("Before Sort -------------------------------------")
		#print("play piece index = ", list_of_plays_simulated[score_max_index].player_index)
		#print("play direction = ", list_of_plays_simulated[score_max_index].direction)
		#print("play force_lerp = ", list_of_plays_simulated[score_max_index].force_lerp)
		#print("play selected score = ", list_of_plays_simulated[score_max_index].score)
		...
		#print("After Sort -------------------------------------")
		#print("play piece index = ", list_of_plays_simulated_Ordered[0].player_index)
		#print("play direction = ", list_of_plays_simulated_Ordered[0].direction)
		#print("play force_lerp = ", list_of_plays_simulated_Ordered[0].force_lerp)
		#print("play selected score = ", list_of_plays_simulated_Ordered[0].score)
	...
	for play in list_of_plays_simulated_Ordered:
		print("list_of_plays_simulated_Ordered -------------------------------------")
		print("play piece index = ", play.player_index)
		print("play direction = " ,play.direction)
		print("play force_lerp = ", play.force_lerp)
		print("play selected score = ", play.score)
...
func Set_All_Plays_To_Simulate_AroundPlayer_Mode(_rotation_angle_steps: int, play_teamSide: int, play_index: int) -> void:
	...
	for k in range(1, max_force_steps + 1):
		...
		if force_lerp == 0:
			print("AAAAAAAAAAAA")
			print("k = ", k)
			print("_max_force_steps = ", max_force_steps + 1)
