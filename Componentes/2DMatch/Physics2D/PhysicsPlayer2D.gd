...
func Set_Current_Velocity(new_velocity: Vector2) -> void:
	current_velocity = Vector2(new_velocity.x, new_velocity.y)
...
#
		...
		#
		#var vetor_arrasto_2d = Vector2(direcao_atual.x, direcao_atual.y)
...
func get_world_pos_from_screen_pos(screen_position: Vector2) -> Vector2:
	...
	var query = PhysicsRayQueryParameters2D.create(ray_from, ray_to, pow(2, 3 - 1))
...
#var base_visual_rotation: Vector2 = Vector2.ZERO
#
...
#
		...
		#visual_piece.rotation = base_visual_rotation + Vector2(
...
#
	#var direcao_arrasto := vetor_arrasto_2d.normalized()
		...
		#return
#
	#var direcao_local := Vector2(direcao_arrasto.x, 0, direcao_arrasto.y).normalized()
...
#
	...
	#var basis_base := Basis(Vector2.UP, base_rotation_y)
...
#
	...
	#smoke_instance_atual.look_at(smoke_instance_atual.global_position + look_dir.normalized(), Vector2.UP)
#
...
#
#func ativar_spark_no_ponto_global(ponto_global: Vector2) -> void:
	#if spark_scene == null:
		#return
#
...
#func _desenhar_mira() -> void:
		...
		#mira_pivot.look_at(ponto_alvo, Vector2.UP)
#
