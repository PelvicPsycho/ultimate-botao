extends Node
class_name CollisionResolution2D_Tutorial_2

@export var match_state: MatchState_Tutorial_2
...
func _physics_process(_delta: float) -> void:
	if all_physicObjects_loaded and current_pitch_state_loaded:
		...
		#for i in PhysicsObjects_List.size():
			#print("A current_velocity = ", PhysicsObjects_List[i].current_velocity)
